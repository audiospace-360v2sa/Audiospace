...
def extract_embeddings(wav_files, channels, samplingrate, content_type, openl3_hop_size, batch_size=16):
    """
    ...
    for i in tqdm(range(0, len(wav_files), batch_size)):
        ...
        emb, _ = openl3.get_audio_embedding(batch_audio_l, batch_sr, model=model, verbose=False,
                                            hop_size=openl3_hop_size, batch_size=batch_size)
        ...
        # extracting stereo embeddings (dim=1024), since we concatenate L (dim=512) and R (dim=512) embeddings
            ...
            emb_r, _ = openl3.get_audio_embedding(batch_audio_r, batch_sr, model=model, verbose=False,
                                                  hop_size=openl3_hop_size, batch_size=batch_size)
...
def extract_embeddings_nobatching(wav_files, channels, samplingrate, content_type, openl3_hop_size):
    """
    ...
    for file in tqdm(wav_files):
        ...
        # or extracting mono embeddings (dim=512)
            ...
            emb, _ = openl3.get_audio_embedding(audio, samplingrate, model=model, verbose=False,
...
def openl3_fd(ids, channels, samplingrate, content_type, openl3_hop_size, eval_path,
              eval_files_extension='.wav', ref_path=None, ref_files_extension='.wav', load_ref_embeddings=None,
              batching=False):
    """
    ...
    if load_ref_embeddings:
        ...
        if ref_path:
            ...
            print('[EXTRACTING REFERENCE EMBEDDINGS] ', ref_path)
            wav_files = []
            for id in ids:
                wav_files.append(os.path.join(ref_path, id + ref_files_extension))
            if batching:
                ref_embeddings = extract_embeddings(wav_files, channels, samplingrate, content_type, openl3_hop_size,
                                                    batch_size=batching)
            else:
                ref_embeddings = extract_embeddings_nobatching(wav_files, channels, samplingrate, content_type,
                                                               openl3_hop_size)
    ...
    print('[EXTRACTING EVALUATION EMBEDDINGS] ', eval_path)
    wav_files = []
    for id in ids:
        wav_files.append(os.path.join(eval_path, id + eval_files_extension))
    if batching:
        eval_embeddings = extract_embeddings(wav_files, channels, samplingrate, content_type, openl3_hop_size,
                                             batch_size=batching)
    else:
        eval_embeddings = extract_embeddings_nobatching(wav_files, channels, samplingrate, content_type,
                                                        openl3_hop_size)

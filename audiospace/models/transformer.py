...
from .blocks import ChannelLastConv1d, ConvMLP, MLP, GatedFusion
        ...
        **kwargs
        ...
        if use_conv:
            self.project_in = nn.Sequential(
                    ChannelLastConv1d(dim_in, dim, kernel_size=7, padding=3),
                    nn.SiLU(),
                    ConvMLP(dim, dim, kernel_size=7, padding=3),
                )
            # self.project_out = ChannelLastConv1d(dim, dim_out, bias=False, kernel_size=7, padding=3) if dim_out is not None else nn.Identity()
        elif use_mlp:
            self.project_in = nn.Sequential(
    ...
    def forward(
        ...
        **kwargs
        ...
        if add_cond is not None:
            if self.use_fusion:
                x = self.gated_fusion(add_cond, x)
            else:
                x = x + add_cond
